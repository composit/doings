...
describe Client do
  ...
  it "should require an address" do
    client = Factory.build( :client, address: nil )
    client.should_not be_valid

    client.errors.should eql( address: ["can't be blank"] )
  end
  ...
  it "should set itself as the billable for its billing rate if it is not set" do
    client = Client.create( name: "Test client", created_by_user: Factory( :user ), address: Factory( :address ), billing_rate_attributes: { units: "month", dollars: 100 } )
